...
void sparse_to_compact(int sparse[max][max], int compact[max*max+1][3], int row, int col)
{
	int a = 1;
...
void fast_transpose(int mat1[max*max+1][3],int mat2[max*max+1][3])
{
	...
	for(int i = 1; i <= t; i++)
	{
		temp2[i] = temp2[i-1] + temp1[i-1];			// Storing Location for element et column
	}
...
int main()
{
	...
	do
	{
		...
		printf("\n\n\n **********  MENU  **********\n\n");
		printf(" 1. Accept a new Sparse Matrix \n");                      // Printing Menu for Sparse Operations
		...
		switch(ch)
		{
			...
			case 2 : sparse_to_compact(sparse_matrix,compact_matrix1,rows, columns);
					 break;

			case 3 : simple_transpose(compact_matrix1,compact_matrix2);
					 break;

			case 4 : fast_transpose(compact_matrix1,compact_matrix2);
					 break;
