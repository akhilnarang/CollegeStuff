...
def solve_recursive(board, column):
    ...
    for i in range(n):
        ...
        if (left_diagonal[i - column + n - 1] != 1 and right_diagonal[i + column] != 1) and column[i] != 1:

            # Place this queen in board[i][column]
            board[i][column] = 1
            left_diagonal[i - column + n - 1] = right_diagonal[i + column] = column[i] = 1
            ...
            board[i][column] = 0 
            left_diagonal[i - column + n - 1] = right_diagonal[i + column] = column[i] = 0
